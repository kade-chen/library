...
var (
	...
	impl invoice.Service
)

func TestMain1(t *testing.T) {
	fmt.Println(ioc.Controller().List())
	a, _ := impl.QueryByDateProjectServicesAll(ctx, &model.ProjectDataServiceSkuRequest{})
...
func init() {
	...
	impl = ioc.Controller().Get(services.AppName).(invoice.Service)
}
