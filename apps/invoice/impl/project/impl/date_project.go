...
func (s *service) QueryByDateProject(ctx context.Context, config *model.ProjectDataRequest) ([]model.ProjectDateCost, error) {
	...
	s.log.Info().Msgf("trances_id=%s, The User begins Query for InvoicMonthByDatePojectAPI", ctx.Value("trances_id"))
	// 构造查询
	...
	for {
		...
		if err == iterator.Done {
			s.log.Info().Msgf("trances_id=%s, Query completed for InvoicMonthByDatePojectAPI", ctx.Value("trances_id"))
			break
		}
		if err != nil {
...
// 辅助方法
func (s *service) QueryByDateProjectAllServicesAllSkus(ctx context.Context, config *model.ProjectDataServiceSkuRequest) (model.ByDateProjectAllServicesSkusList, error) {
