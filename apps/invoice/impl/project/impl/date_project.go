...
func (s *service) QueryByDateProject(ctx context.Context, config *model.ProjectDataRequest) ([]model.ProjectDateCost, error) {
	...
	s.log.Info().Msgf("trances_id=%s, The User begins Query for InvoicMonthByDatePojectAPI", trancesID)
	// 构造查询
	s.log.Info().Msgf("trances_id=%s, Retrieving initialization SQL......", trancesID)
	sql := s.queryByDateProjectSQL1(config.OrganizationBqTable)
	...
	for {
		...
		if err == iterator.Done {
			s.log.Info().Msgf("trances_id=%s, Query completed for InvoicMonthByDatePojectAPI", trancesID)
			break
		}
		if err != nil {
