...
func (s *service) QueryByProject(ctx context.Context, config *model.ProjectRequest) ([]model.ProjectCost, error) {
	...
	s.log.Info().Msgf("trances_id=%s, The User begins Query for InvoiceMonthByPojectAPI", trancesID)
	// 构造查询
	...
	for {
		...
		if err == iterator.Done {
			s.log.Info().Msgf("trances_id=%s, Query completed for InvoiceMonthByPojectAPI", trancesID)
			break
		}
		if err != nil {
