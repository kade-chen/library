...
func (s *service) QueryByProject(ctx context.Context, config *model.ProjectRequest) ([]model.ProjectCost, error) {
	...
	s.log.Info().Msgf("trances_id=%s, The User begins Query for InvoiceMonthByPojectAPI", ctx.Value("trances_id"))
	// 构造查询
	s.log.Info().Msgf("trances_id=%s, Retrieving initialization SQL......", ctx.Value("trances_id"))
	sql := s.queryByProjectSQL()
	...
	prev_start, prev_end := tools.InvoiceMonthTime(config.StartDate, config.EndDate)
	...
	prev_PartitionStartTime, prev_PartitionEndTime := tools.InvoiceMonthPartitionTime(prev_start, prev_end)
	s.log.Info().Msgf("trances_id=%s, prev_start_date=%v, prev_end_date=%v, prev_PartitionStartTime=%v, prev_PartitionEndTime=%v", ctx.Value("trances_id"), prev_start, prev_end, prev_PartitionStartTime, prev_PartitionEndTime)
	...
	for {
		...
		if err == iterator.Done {
			s.log.Info().Msgf("trances_id=%s, Query completed for InvoiceMonthByPojectAPI", ctx.Value("trances_id"))
			break
		}
		if err != nil {
		...
		if !row.ProjectID.Valid {
			s.log.Info().Msgf("trances_id=%s, ProjectID is null, setting to '[Charges not specific to a project]", ctx.Value("trances_id"))
