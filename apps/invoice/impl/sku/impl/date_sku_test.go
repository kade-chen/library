...
var (
	...
	impl invoice.SkuService
)

func TestSKU(t *testing.T) {
	fmt.Println(ioc.Controller().List())
	a, _ := impl.QueryByDateProjectSKUsAll(ctx, &model.ProjectDataServiceSkuRequest{})
...
func init() {
	...
	impl = ioc.Controller().Get(sku.AppName).(invoice.SkuService)
}
