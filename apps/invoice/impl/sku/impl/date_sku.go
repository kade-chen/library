...
func (s *service) QueryByDateProjectSKUsAll(ctx context.Context, config *model.ProjectDataServiceSkuRequest) ([]model.SkusList, error) {
	...
	s.log.Info().Msgf("trances_id=%s, The User begins Query for InvoiceMonthByDatesServiceSkusAPI", trancesID)
	// 构造查询
	...
	for {
		...
		if err == iterator.Done {
			s.log.Info().Msgf("trances_id=%s, Query completed for InvoiceMonthByDatesServiceSkusAPI", trancesID)
			break
		}
		if err != nil {
...
func (s *service) QueryByDateSku(ctx context.Context, config *model.SkuDataRequest) ([]model.SkuDateCost, error) {
	...
	for {
		...
		err := it.Next(&row)
		if err == iterator.Done {
			s.log.Info().Msgf("trances_id=%s, Query completed for InvoiceMonthByDateSkuAPI", trancesID)
			break
		}
		if err != nil {
			s.log.Error().Msgf("trances_id=%s, Failed to iterate over query results: %v", trancesID, err)
