...
func (s *service) QueryByDateProjectSKUsAll(ctx context.Context, config *model.ProjectDataServiceSkuRequest) ([]model.SkusList, error) {
	...
	s.log.Info().Msgf("trances_id=%s, The User begins Query for InvoiceMonthByDatesServiceSkusAPI", ctx.Value("trances_id"))
	// 构造查询
	...
	for {
		...
		if err == iterator.Done {
			s.log.Info().Msgf("trances_id=%s, Query completed for InvoiceMonthByDatesServiceSkusAPI", ctx.Value("trances_id"))
			break
		}
		if err != nil {
...
func (s *service) QueryByDateSku(ctx context.Context, config *model.SkuDataRequest) ([]model.SkuDateCost, error) {
	...
	s.log.Info().Msgf("trances_id=%s, The User begins Query for InvoiceMonthByDateSkuAPI", ctx.Value("trances_id"))
	// 构造查询
	s.log.Info().Msgf("trances_id=%s, Retrieving initialization SQL......", ctx.Value("trances_id"))
	...
	for {
		...
		err := it.Next(&row)
		if err == iterator.Done {
			s.log.Info().Msgf("trances_id=%s, Query completed for InvoiceMonthByDateSkuAPI", ctx.Value("trances_id"))
			break
		}
		if err != nil {
			s.log.Error().Msgf("trances_id=%s, Failed to iterate over query results: %v", ctx.Value("trances_id"), err)
