...
func (s *service) QueryBySku(ctx context.Context, config *model.SkuRequest) ([]model.SkuCost, error) {
	...
	s.log.Debug().Msgf("start_date: %s, end_date: %s", config.StartDate, config.EndDate)
	s.log.Debug().Msgf("prev_start: %s, prev_end: %s", prev_start, prev_end)
	s.log.Debug().Msgf("partitionStartTime: %s, partitionEndTime: %s", partitionStartTime, partitionEndTime)
	s.log.Debug().Msgf("prev_PartitionStartTime: %s, prev_PartitionEndTime: %s", prev_PartitionStartTime, prev_PartitionEndTime)
	// 绑定参数
