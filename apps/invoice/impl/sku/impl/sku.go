...
func (s *service) QueryBySku(ctx context.Context, config *model.SkuRequest) ([]model.SkuCost, error) {
	...
	s.log.Info().Msgf("trances_id=%s, The User begins Query for UsageDateBySkuAPI", trancesID)
	// 构造查询
	s.log.Info().Msgf("trances_id=%s, Retrieving initialization SQL......", trancesID)
	sql := s.queryBySkuSQL1(config.OrganizationBqTable)
	...
	for {
		...
		if err == iterator.Done {
			s.log.Info().Msgf("trances_id=%s, Query completed for UsageDateBySkuAPI", trancesID)
			break
		}
		if err != nil {
