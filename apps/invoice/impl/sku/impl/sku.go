...
func (s *service) QueryBySku(ctx context.Context, config *model.SkuRequest) ([]model.SkuCost, error) {
	if ctx.Value("trances_id") == nil {
		ctx = context.WithValue(context.Background(), "trances_id", trances.NewTraceID())
	}
	s.log.Info().Msgf("trances_id=%s, The User begins Query for UsageDateBySkuAPI", ctx.Value("trances_id"))
	// 构造查询
	...
	var results []model.SkuCost
	for {
		var row model.SkuCost
		err := it.Next(&row)
		if err == iterator.Done {
			s.log.Info().Msgf("trances_id=%s, Query completed for UsageDateBySkuAPI", ctx.Value("trances_id"))
			break
		}
		if err != nil {
