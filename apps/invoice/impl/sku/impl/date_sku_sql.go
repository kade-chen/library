...
func (s *service) queryByDateSkuSQL2(table string) (sql string) {
	// 查询全部项目
	sql = `
		...
		SELECT
			...
			usage_amount,
			usage_pricing_unit,
			usage_amount_details,
			usage_pricing_unit_details,
...
func (s *service) queryByDateSkuSQL1() (sql string) {
	// 查询全部项目
	sql = `
		WITH cost_summary AS (
		    SELECT
		...
		SELECT
		    ...
		    -- Sub_Total
		    cost_at_list + cost - cost_at_list + Savings_Programs + Other_Savings AS Sub_Total,
			ROUND(cost_at_list + cost - cost_at_list + Savings_Programs + Other_Savings , 2) AS Subtotal
		FROM cost_summary
		ORDER BY usage_date DESC, sku_id;
			`
