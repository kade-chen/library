...
func (s *service) queryByDateServiceSQL() (sql string) {
	...
	sql = `
		...
		FROM cost_summary a
		LEFT JOIN service_summary b USING (usage_date, service_description)
		LEFT JOIN Saving_summary c USING (usage_date, service_description)
		LEFT JOIN other_summary d USING (usage_date, service_description)
		ORDER BY a.usage_date DESC, a.service_description;
			`

	return sql
}

func (s *service) queryByDateServiceSQLBackup() (sql string) {
	// 查询全部项目
	sql = `
		WITH cost_summary AS (
		  SELECT
