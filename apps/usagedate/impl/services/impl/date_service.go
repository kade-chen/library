...
func (s *service) QueryByDateProjectServicesAll(ctx context.Context, config *model.ProjectDataServiceSkusRequest) ([]model.ServicesList, error) {
	...
	sql := s.queryByDateProjectSUSQ1L(config.BqTable)
...
func (s *service) QueryByDateService(ctx context.Context, config *model.ServiceDataRequest) ([]model.ServiceDateCost, error) {
	...
	s.log.Info().Msgf("trances_id=%s, The User begins Query for UsageDateByDateServiceAPI", ctx.Value("trances_id"))
	// 构造查询
	...
	for {
		...
		err := it.Next(&row)
		if err == iterator.Done {
			s.log.Info().Msgf("trances_id=%s, Query completed for UsageDateByDateServiceAPI", ctx.Value("trances_id"))
			break
		}
		if err != nil {
