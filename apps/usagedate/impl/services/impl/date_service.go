...
func (s *service) QueryByDateService(ctx context.Context, config *model.ServiceDataRequest) ([]model.ServiceDateCost, error) {
	...
	// }
	trancesID := ctx.Value("claims").(*authModel.TokenAuthMiddleware).TrancesID
	s.log.Info().Msgf("trances_id=%s, The User begins Query for UsageDateByDateServiceAPI", trancesID)
	// 构造查询
	s.log.Info().Msgf("trances_id=%s, Retrieving initialization SQL......", trancesID)
	...
	for {
		...
		err := it.Next(&row)
		if err == iterator.Done {
			s.log.Info().Msgf("trances_id=%s, Query completed for UsageDateByDateServiceAPI", trancesID)
			break
		}
		if err != nil {
			s.log.Error().Msgf("trances_id=%s, Failed to iterate over query results: %v", trancesID, err)
