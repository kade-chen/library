...
func (s *service) QueryByUsageDatProjectLabelKeyAll(ctx context.Context, config *model.UsageDateProjectLabelKeyRequest) (model.UsageDateProjectLabelKeyLists, error) {
	...
	if err != nil {
		...
		return model.UsageDateProjectLabelKeyLists{}, exception.NewInternalServerError("trances_id=%s, Failed to execute query: %v", ctx.Value("trances_id"), err)
	}
	...
	for {
		...
		if err != nil {
			...
			return model.UsageDateProjectLabelKeyLists{}, exception.NewInternalServerError("trances_id=%s, Failed to iterate over query results: %v", ctx.Value("trances_id"), err)
		}
