...
func (s *service) QueryByDateSku(ctx context.Context, config *model.SkuDataRequest) ([]model.SkuDateCost, error) {
	...
	s.log.Info().Msgf("trances_id=%s, The User begins Query for UsageDateByDateSkuAPI", ctx.Value("trances_id"))
	// 构造查询
	...
	for {
		...
		err := it.Next(&row)
		if err == iterator.Done {
			s.log.Info().Msgf("trances_id=%s, Query completed for UsageDateByDateSkuAPI", ctx.Value("trances_id"))
			break
		}
		if err != nil {
