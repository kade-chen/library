...
func (s *service) queryBySkuSQL1(table string) (sql string) {
	// 查询全部项目
	sql = `
		WITH cost_summary AS (
		    SELECT
		        ...
		        sku.id AS sku_id, 
		        IFNULL(SUM(usage.amount_in_pricing_units), 0) AS usage_amount,
		        ...
		        IFNULL(SUM(usage.amount), 0) AS usage_amount_details,
		...
		cost_summary_last AS (
		    ...
		    WHERE
              	...
              	AND usage_start_time BETWEEN TIMESTAMP(@prev_PartitionStartTime) AND TIMESTAMP(@prev_PartitionEndTime)
              	AND (ARRAY_LENGTH(@project_ids) IS NULL OR ARRAY_LENGTH(@project_ids) = 0 OR project.id IN UNNEST(@project_ids))
              	AND (ARRAY_LENGTH(@services_ids) IS NULL OR ARRAY_LENGTH(@services_ids) = 0 OR service.id IN UNNEST(@services_ids))
              	AND (ARRAY_LENGTH(@skus_ids) IS NULL OR ARRAY_LENGTH(@skus_ids) = 0 OR sku.id IN UNNEST(@skus_ids))
				AND (ARRAY_LENGTH(@keys) IS NULL OR ARRAY_LENGTH(@keys) = 0 OR labels[SAFE_OFFSET(0)].key IN UNNEST(@keys))
		...
		SELECT
		    ...
		    a.sku_id,
			a.usage_amount,
			a.usage_pricing_unit,
			a.usage_amount_details,
			a.usage_pricing_unit_details,
	...
	return fmt.Sprintf(sql, table, table)
}
