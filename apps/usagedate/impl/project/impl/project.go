...
func (s *service) QueryByProject(ctx context.Context, config *model.ProjectRequest) ([]model.ProjectCost, error) {
	...
	s.log.Info().Msgf("trances_id=%s, The User begins Query for UsageDateByPojectAPI", ctx.Value("trances_id"))
	// 构造查询
	...
	for {
		...
		if err == iterator.Done {
			s.log.Info().Msgf("trances_id=%s, Query completed for UsageDateByPojectAPI", ctx.Value("trances_id"))
			break
		}
		if err != nil {
